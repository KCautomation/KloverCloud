...
class TestCreatePHP(EnvironmentSetup):
    ...
    def test1(self):
        pageUrl = "https://eks.rakibefstestmaincluster782.klovercloud.io/auth/login"
        driver = self.driver
        ...
        if Laravel.is_displayed():
            Laravel.click()
            print("Choose Laravel")
            time.sleep(2)
        else:
            print("Laravel is no displayed")
        ...
        ss = SS(driver)
        file_name = ss_path + "PHP_scrrenshot_" + time.asctime().replace(":", "_") + ".png"
